...
class Pathfinder3D:
    ...
    def find(self, start: Tuple[float, float, float],
        ...
        # 8-connectivity in xy + 3 options for theta (stay, +, -)
            ...
            for dr, dc in neighbors_xy:
                for dk in neighbors_theta:
                    ...
                    if neighbor not in g_score or tentative_g < g_score[neighbor]:
                        ...
                        # Heuristic: spatial + angular distance to goal
                        h_spatial = np.sqrt((nr - goal_cell[0])**2 + (nc - goal_cell[1])**2) * self.cspace.resolution
                        h_theta = min(abs(nk - goal_cell[2]), self.cspace.n_angles - abs(nk - goal_cell[2])) * self.cspace.angle_resolution * 0.5
                        heapq.heappush(open_set, (tentative_g + h_spatial + h_theta, neighbor))
